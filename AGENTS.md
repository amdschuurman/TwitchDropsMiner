...
- REST API endpoints: `/api/status`, `/api/channels`, `/api/campaigns`, `/api/settings`, `/api/login`, `/api/oauth/confirm`, `/api/reload`, `/api/close`, `/api/version`, `/api/health/mining` (readiness probe for an external uptime monitor: always HTTP 200, `ok` false while there is nothing to mine - the watch list is empty (`watchlist_empty`) or no benefit type is enabled (`benefits_disabled`), which are reported separately because they need different fixes - or while nobody is logged in (`login_pending` marks a device-code login nobody has approved yet, which waits forever), or while the miner has games it wants but has not landed a watch tick for several minutes (`watch_stalled`, with `last_watch_age_seconds` and `uptime_seconds` so a stall is distinguishable from a fresh start), or while the miner is still starting, or a state read failed. `wanted_games_count == 0` alone is NOT an error: an account with games configured and no active campaign right now is the ordinary state for days at a time, and alerting on it would train the owner to ignore the alert. Public by EXACT path in the auth gate allowlist, so `/api/health/mining/` with a trailing slash is gated and answers 401, which a monitor inverting a match on `"ok":false` reads as healthy forever. The body is limited to booleans, counts, a coarse `state` of `starting` | `idle` | `watching` | `paused`, the Twitch login, and the progress fields above. `watching` means watching a channel *for drops*, so an idle watch (a channel held open for channel points or predictions while nothing is minable) reports `idle`, and `mining` follows `state`; a held `watching_channel` is not on its own proof of mining. `ok` does not drop on `idle`. No channel or campaign names: `state` is derived from the miner objects, never from the status line, which embeds both)
...
- `mining_benefits` and `scheduler_start` / `scheduler_stop`: the other two settings whose empty or unreadable state silently stops all mining, guarded the same way, see Settings Write Invariants below
...
The shape they all share, and the thing to look for when adding a setting: a value whose *empty* or *unreadable* state means "mine nothing" but raises nothing, logs nothing and leaves the dashboard looking healthy. `games_to_watch` was the first one found. `mining_benefits` reaches the same standstill through `Benefit.is_wanted` being fail-closed, and the scheduler boundaries reach it by killing the only task that can lift a pause. Each of those gets the same two-sided treatment: a payload guard in `src/web/managers/settings.py` so no client can write the state by accident, and a floor on the STORED value in `Settings.save` so nothing can write it with no client involved at all. The two halves share one definition of "this value leaves nothing to mine" (`MiningFloor.enables_nothing`) precisely so they cannot drift into "HTTP 200 for a change that never reached disk".
...
- **A request that empties `games_to_watch` must also prove it is not stale.** The intent flag says "emptying is deliberate", not "emptying THIS list is deliberate", and only the second one is safe: `saveSettings` POSTs the client's entire list as authoritative and every single-game-removal gesture carries the flag, so a tab that loaded while the list was `['A']` sends a request byte-identical to a user clearing `['A','B','C']`. So a clearing request also carries `expected_games_to_watch`, the list that tab believed was stored, compared in ORDER (list order is mining order in `src/services/stream_selector.py`) and refused on any mismatch with `Refused to clear the games-to-watch list: this request expected ... but the stored list is ...`. A refusal does NOT declare the save-side intent. The check is skipped when the stored list is already empty - nothing can be lost, and demanding a match there would refuse a fresh install's first "clear all" and the second gesture of a real one, whose `settings_updated` broadcast (`asyncio.create_task`, so it lands later) has not arrived yet. Non-empty writes are untouched by any of this. `expected_games_to_watch` is request-only, like the flags. Client side: `state.serverGamesToWatch` is snapshotted in `updateSettingsUI` as a COPY - the gestures splice `state.settings.games_to_watch` in place, so an alias would compare the edited list against itself - and refreshed again from every settings POST response, because the broadcast is too late to close the window. A refused clear is announced with `addConsoleLine` and the tab resyncs; a silent refusal looks exactly like the bug.
- **`mining_benefits` is merged onto the stored selection, never substituted for it, and may not be emptied unasked.** `Benefit.is_wanted` is fail-CLOSED (`allowed_benefits.get(name, False)`), so an omitted key is not "leave it as it was", it is "turn it off" - and the dashboard builds the dict from four `getElementById(...)?.checked` reads, each of which yields `undefined` for an unrendered element, which `JSON.stringify` drops. A partial payload therefore disabled benefit types nobody touched. `_BenefitsGuard.resolve` merges instead, which gives an absent key the same meaning `check_and_update_setting` already gives every absent value, and drops keys outside `MINING_BENEFIT_KEYS` and non-bool values (`merge_json` would drop them on the next load anyway). Emptying is judged on the MERGED result, needs `allow_empty_mining_benefits` on the same request, and is refused otherwise with `Refused to disable every mining benefit type without explicit intent`. `MINING_BENEFIT_KEYS` is derived from `default_settings`, never hand-listed. `GET /api/health/mining` reports `benefits_disabled` and folds it into `ok`, because this state used to answer `ok: true` to the monitor installed to catch exactly this outage.
- **`scheduler_start` / `scheduler_stop` are validated on write and repaired at load.** `SchedulerService._parse_time` had no guard and nothing caught it: `""` and `"banana"` raise ValueError from `int`, `"22"` raises IndexError, `"24:00"` and `"-1:00"` raise ValueError from `time`, and the exception left `run_scheduler`'s loop, which kills the task for the rest of the process. If the scheduler had already paused the miner, nothing was left to resume it - mining stopped with no console line and a health probe still reporting `ok`, because a paused miner is legitimate. `ClockTime` (`src/config/settings.py`) owns the parse; `SettingsManager._validate_clock_time` guards the write and `Settings.load` repairs a stored value, for the same reason `language` needs both (the equality short-circuit means a poisoned stored value is never revalidated by the UI posting it back, and `merge_json` only checks that it is a `str`). `ClockTime` is deliberately at least as strict as the parser it guards, never looser. The repair does NOT also switch `scheduler_enabled` off: that would mutate a setting the operator did choose, on evidence about a different one, and the next save would persist the loss.
...
- **Every settings, cache and cookie write is atomic.** `json_save` (`src/utils/json_utils.py`) serializes into a temporary file in the target's own directory, `fsync`s it, then `os.replace`s it onto the target. Its callers are `settings.json` (`src/config/settings.py`) and the channel-point caches (`src/services/message_handlers.py`, `src/web/app.py`). It must never truncate in place: `open(path, "w")` empties the file before the new bytes land, so an interrupted save left a truncated `settings.json` behind, and `json_load(..., default_settings, merge=True)` then falls back to the defaults, whose `games_to_watch` is `[]`. That is the same silent outage reached without any client, endpoint or cleaner being involved. The temporary file also carries the 0o600 mode onto the target, which is what keeps credential-bearing proxy URLs unreadable to other users on the host, and a successful save reaps this target's own abandoned temp files once they are older than an hour.

  `cookies.jar` is atomic too, but by its own route, not `json_save`'s: `CookieJarStore` (`src/api/http_client.py`) wraps `aiohttp.CookieJar.save()` - which is `open(path, "wb")` plus `pickle.dump`, and so truncates in place - in the same mkstemp/fsync/`os.replace` shape, and both call sites use it (`HTTPClient.close`, `_AuthState._validate`). Do not route the jar through `json_save` to unify them: the file is aiohttp's pickle format, not JSON, and the format stays aiohttp's to own. The 0o600 mode now lands on the temp file before the auth-token is written to it, replacing the post-hoc `chmod` that left the token briefly world-readable, and pre-existing loose modes are tightened by the replace. The store also reaps its own stale temps, at startup as well as after a landed write, because the jar is written only on login and clean shutdown. A failed jar write is reported at ERROR and does NOT propagate: it used to abort `_validate()`, which every GQL request waits on through `Twitch.get_auth()`, so a full disk at login stopped mining entirely over a file only needed at the next start - the atomic write is what makes carrying on safe, since the previous jar is left whole. An unreadable jar degrades to logged-out and is reported at WARNING; deliberately NOT quarantined the way settings.json is, because it is a regenerable credential and keeping `.corrupt` copies of an auth-token adds exposure for nothing.
...
- **"Unparseable" means structurally, not just syntactically.** `json_load` promises a dict, so a file that is valid JSON but is not one - `[]`, `42`, `"hello"`, `null`, a `__type` wrapper that decodes to a set - is corrupt in exactly the sense above and takes the same route. It used to reach `_remove_missing` and die there with `'list' object has no attribute 'items'`, as did a recognized `__type` wrapper whose payload its constructor rejects (`{"__type": "set", "data": 5}` -> TypeError, `{"__type": "set"}` -> KeyError, `{"__type": "datetime", "data": 1e300}` -> OverflowError). None of those was caught anywhere: for settings.json that is `sys.exit(4)` on every boot and a restart loop that quarantines nothing and reports nothing. Both points raise `_UnusableJSONError`, a `ValueError` subclass so the existing handler catches it, and both are raised narrowly - catching the underlying `AttributeError`/`TypeError`/`KeyError` where they surfaced would swallow the same types coming out of a genuine bug. An unrecognized `__type` wrapper inside a LIST refuses the whole file rather than dropping the element: a dict can lose a key because `merge_json` puts the default back, a list has no per-element template, and dropping one would shorten `games_to_watch` by a game silently - which is this whole document. Leaving it was worse: the bare sentinel survived into memory and made `json_save` raise `TypeError` on that save and every save after it. Messages name the wrapper's TYPE and never its data, which can be a proxy or webhook URL.
- **An empty template in `merge_json` is an OPEN MAP, not a schema.** A non-empty template enumerates every key that may exist, so a stored key it does not list is a setting this version removed and pruning it is right. An empty one enumerates nothing, so it cannot be an authority on what is unknown and every stored key is the operator's data. `channel_strategies` is one (`{}` in `default_settings`, one entry per channel the user picked a betting strategy for) and the old rule deleted all of them on load, then let the next save persist `{}`: the empty watch list again, one setting over. An empty template therefore short-circuits - no prune, no type check, no key fill. Both destructive branches are now logged at ERROR, the removals as ONE line per object rather than one per key so a legacy file yields a line and not a wall. Keys are safe to print precisely BECAUSE open maps are never pruned: every deletable key is a setting name, never something the user typed. Values are still never printed.
- **Quarantined files are reaped, timidly.** Nothing ever collected them, so a filesystem that corrupts settings.json on every boot fills the data directory with `.corrupt`, `.corrupt.1` ... `.corrupt.99` and then unbounded timestamped names, at 100 failed `stat` calls each once the slots are gone. The reaper waits thirty days rather than the temp sweep's hour, and spares the OLDEST quarantine whatever its age - the same "an earlier quarantine outranks a later one" rule `quarantine_corrupt` follows when it refuses to clobber one, because the earliest copy is the likeliest to hold real pre-corruption data. `quarantine_corrupt` re-stamps the preserved copy's mtime, which is load-bearing: `os.replace` carries the original across, so a settings.json last saved months before it was corrupted would land already looking ancient and be reaped on the spot.
...
- `tests/test_settings_api.py` - Tests for the settings write path and the stored-state invariants above: the `games_to_watch` clear guard on the payload, the stale-tab check under it (`expected_games_to_watch`) and the save-side floor under that, booting from a quarantined `settings.json`, language validation, the load-time language repair, the accepts/knows split, and `LanguageNormalizer.apply` at startup
- `tests/test_mining_benefits_guard.py` - Tests the other road to "nothing to mine": that a partial `mining_benefits` payload is merged rather than substituted, that disabling every benefit type needs explicit intent on both the payload and the save side, and that the two floors are independent
- `tests/test_scheduler_boundary.py` - Tests that an `HH:MM` the scheduler cannot read is rejected on write and repaired at load, and that `ClockTime` and `SchedulerService` cannot drift apart
- `tests/test_json_atomic_save.py` - Tests that `json_save` is crash-atomic, keeps the file 0o600 and reaps its own stale temp files; that `json_load(..., quarantine=True)` preserves an unparseable file - including one that is valid JSON of the wrong shape - instead of letting the next save overwrite it; that `merge_json` reports both destructive branches and leaves open maps alone; and that quarantines are eventually reaped without ever losing the oldest
- `tests/test_cookie_jar_atomic.py` - Tests that `CookieJarStore` writes the jar atomically at 0o600 with the mode set before the token lands, degrades loudly on an unreadable jar, and that a failed jar write stops neither the login nor the shutdown
- `tests/test_health_mining_api.py` - Tests for `GET /api/health/mining`: its public key set (`watchlist_empty` and `benefits_disabled` both folded into `ok`), its coarse `state` (including that an idle watch is not mining), that it is public at that exact path only, what it must not disclose, and that the alert keyword README documents matches the bytes on the wire
