...
(function consumeBootstrapToken() {
    ...
    // API_BASE keeps multi-instance (?acc=N -> /accN) setups hitting the right backend.
    window.location.replace(API_BASE + '/api/session/bootstrap?token=' + encodeURIComponent(token));
})();

// Initialize Socket.IO connection. withCredentials forces the session cookie to
// be sent with the handshake even when the page is served over a non-default port.
...
function updateLoginStatus(data) {
    ...
    if (data.user_id) {
        ...
        // Safe DOM construction — `name` comes from the backend payload and must
        // never be interpolated into HTML (XSS guard, see AGENTS.md).
        const loggedInBadge = document.createElement('span');
        loggedInBadge.style.color = 'var(--success-color)';
        loggedInBadge.style.fontWeight = '600';
        loggedInBadge.textContent = `✓ @${name}`;
        statusEl.replaceChildren(loggedInBadge);
...
async function loadInstanceTabs() {
    ...
    try {
        ...
        // fallback: render current instance button only (DOM construction — no innerHTML)
        const fallbackBtn = document.createElement('button');
        fallbackBtn.className = 'acc-tab-btn active-acc';
        fallbackBtn.dataset.accN = ACC_NUM;
        fallbackBtn.textContent = `Account ${ACC_NUM}`;
        container.replaceChildren(fallbackBtn);
    }
}
